...
<?php
session_start();
$nature=$_SESSION['requestNature'];
if($nature=='Settled Claim Requests'){
echo '<select name="requestNature">
        <option>-Select-</option>
        <option value="SettledClaimRequest" selected disabled>Settled Claim Requests </option>
...
echo '<select name="requestNature">
        ...
        <option value="RequestedforClarification" selected disabled>Requested for Clarification</option></select>
...
$_SESSION['fetch1']=$_POST['fetch'];
header("Location:viewclosedrequestdetails1.php");
}
if(isset($_POST['ok'])){
